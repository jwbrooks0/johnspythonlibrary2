...
class keysight_dsos054a:
	...
	*  This code is based on snippets in the keysight programming manual
	...
	def __init__(	self,
					address="TCPIP0::192.168.0.247::inst0::INSTR",
					timeout=60000,
					debug=False):
...
# 			ax.legend()
	...
	# %% save data
	def save_data_locally(self, dir_and_filename='c:\data', file_format='H5INt'):
		"""
		Instructs the scope to write data locally (to its own harddrive or usb drive).
		See page 493 in https://www.keysight.com/us/en/assets/9018-07141/programming-guides/9018-07141.pdf
		Parameters
		----------
		dir_and_filename : str
			The directory and filename (not including the file extension) to write the data to
		file_format : str
			Fileformat of the output data. 
			
			'H5INt' (Default).  HDF5 format saved as ints.  Needs to be multiplied by the y-increment and added to y-origin in order to properly scale to volts
			
			'H5'.  HDF5 format saved as floats.
		Returns
		-------
		None.
		"""
		self.do_command(':DISK:SAVE:WAVeform ALL,"%s",%s,OFF' % (dir_and_filename, file_format))
