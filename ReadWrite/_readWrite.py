...
def datetime_now_to_str(
    ...
    """
    Gets the present time and converts to string.
    I typically use this for timestamping data by putting this str in filenames
    """
...
def str_to_datetime(
    ...
    """
    Converts str (with provided format) to datetime
...
def store_file_locally(
    ...
    """ 
    This function checks to see if the file exists on the local computer.  
    If not there, it downloads it there for the future.  
...
def csv_to_xr(
    ...
    data = csv_to_pd(
        filename=filename, 
        delimiter=delimiter, 
        row_number_of_col_names=row_number_of_col_names, 
...
def append_single_row_to_csv(
        data_row, 
        filename='name.csv', 
        headers=[], 
        delete_file_if_already_exists=False,
        ): 
    """
    Examples
    --------
