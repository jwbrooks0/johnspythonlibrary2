...
def calc_2D_TLPP(s,
    ...
    E = 2
    tlpp = convertToTimeLaggedSpace(s, E=E, tau=tau, fuse=True)
    
    ## create bins for histogram
    if len(bin_min_max) == 0:
        bin_min_max = [s.min(), s.max()]
    N = num_bins
    bins = _np.linspace(bin_min_max[0], bin_min_max[1], N + 1)
    ...
    H = calc_2D_histogram(tlpp, plot=False, bins=bins)
...
def EMD_TLPP_2D(s1, 
    ...
    if plot is True:
        ...
        fig, axes = _plt.subplots(2, 2, gridspec_kw={'width_ratios': [1.25, 1]}, sharey=True)
        ...
        if len(tlim) == 2:
            axes[0, 0].set_xlim(tlim[0], tlim[1])
        
        fig.suptitle("EMD = %.1f" % result)
